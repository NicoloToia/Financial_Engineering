...
FT_1up = FT_1(ST_1>K); %I don't take the first coupon
...
%case C: ST_1<K & ST>K -> we take the first and the third coupon 
ST_C = ST_2down(ST_2down>K); 
probC = length(ST_C)/N; 
partC = principal * (deltas(1) * coupons(1) * coupon_DF(1) + ...
    deltas(3) * coupons (3) * coupon_DF(3)) * probC; 
...
NPV_A = principal * (1 - quarter_DF(4) + s_A * BPV_1y) + ...
    ...
    principal * (quarter_DF(9) - quarter_DF(12) + s_A * BPV_3Y) * (probA+probC); 
